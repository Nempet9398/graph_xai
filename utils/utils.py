...
def visualize_nodes_with_heatmap(data, node_importance):
    ...
    Parameters:
    - data (object): Graph data (torch_geometric.data)
    - node_importance (Tensor): Tensor of node importance scores
    ...
    # Normalize node importance values for coloring
    node_importance = node_importance.cpu().detach().numpy()
    node_colors = [node_importance[i] for i in range(len(G.nodes))]

    nx.draw(G, pos, with_labels=True, node_color=node_colors, edge_color='gray', 
            node_size=1000, font_size=20, cmap=plt.cm.PuBu)

    plt.title("Graph Visualization with Motif-Based GradCAM", fontsize=20)
    plt.colorbar(plt.cm.ScalarMappable(cmap=plt.cm.PuBu), ax=plt.gca(), label='Node Importance')
    plt.show()
...
def visualize_graph_with_clique(test_dataset, index):
    ...
    nx.draw(G, pos, node_color=node_colors, with_labels=True, node_size=300, cmap=cmap, edge_color='gray')
    plt.title('Graph Visualization with Clique Information')
    plt.show()
